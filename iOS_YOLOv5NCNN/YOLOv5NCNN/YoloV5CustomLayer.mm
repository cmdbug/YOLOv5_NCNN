...
YoloV5CustomLayer::YoloV5CustomLayer(bool useGPU) {
#if NCNN_VULKAN
...
#endif
    NSString *parmaPath = [[NSBundle mainBundle] pathForResource:@"yolov5s_customlayer" ofType:@"param"];
    NSString *binPath = [[NSBundle mainBundle] pathForResource:@"yolov5s_customlayer" ofType:@"bin"];
...
std::vector<BoxInfo> YoloV5CustomLayer::detect(UIImage *image, float threshold, float nms_threshold) {
    ...
    ncnn::Mat in_net = ncnn::Mat::from_pixels_resize(rgba, ncnn::Mat::PIXEL_RGBA2RGB, w, h, w, h);
    ...
    for (int i = 0; i < count; i++) {
        ...
        // clip
        x0 = std::max(std::min(x0, (float) (image.size.width - 1)), 0.f);
        y0 = std::max(std::min(y0, (float) (image.size.height - 1)), 0.f);
        x1 = std::max(std::min(x1, (float) (image.size.width - 1)), 0.f);
        y1 = std::max(std::min(y1, (float) (image.size.height - 1)), 0.f);

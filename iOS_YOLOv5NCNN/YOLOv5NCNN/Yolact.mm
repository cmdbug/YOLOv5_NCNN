...
Yolact::Yolact(bool useGPU) {
    ...
    YolactNet->opt.use_vulkan_compute = toUseGPU;
    YolactNet->opt.use_fp16_arithmetic = true;
    NSString *parmaPath = [[NSBundle mainBundle] pathForResource:@"yolact" ofType:@"param"];
    NSString *binPath = [[NSBundle mainBundle] pathForResource:@"yolact" ofType:@"bin"];
    int rp = YolactNet->load_param([parmaPath UTF8String]);
    int rm = YolactNet->load_model([binPath UTF8String]);
    if (rp == 0 && rm == 0) {
...
std::vector<Object> Yolact::detect_yolact(UIImage *image) {
//    https://github.com/Tencent/ncnn/wiki/use-ncnn-with-opencv#ncnn-to-opencv
    ...
    ncnn::Mat in_net = ncnn::Mat::from_pixels_resize(rgba, ncnn::Mat::PIXEL_RGBA2RGB, img_w, img_h, target_size, target_size);
    

    const float mean_vals[3] = {123.68f, 116.78f, 103.94f};
    const float norm_vals[3] = {1.0 / 58.40f, 1.0 / 57.12f, 1.0 / 57.38f};
    in_net.substract_mean_normalize(mean_vals, norm_vals);

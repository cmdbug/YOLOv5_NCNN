...
bool OCR::hasGPU = false;
bool OCR::toUseGPU = false;
...
OCR::OCR(bool useGPU) {
    ...
    NSString *paramPath = [[NSBundle mainBundle] pathForResource:@"dbnet_op" ofType:@"param"];
    NSString *binPath = [[NSBundle mainBundle] pathForResource:@"dbnet_op" ofType:@"bin"];
    int rp = dbnet->load_param([paramPath UTF8String]);
    int rm = dbnet->load_model([binPath UTF8String]);
    if (rp == 0 && rm == 0) {
        NSLog(@"dbnet load param and model success!");
    } else {
        NSLog(@"dbnet load fail,param:%d model:%d", rp, rm);
    }

    crnn_net = new ncnn::Net();
    crnn_net->opt.use_vulkan_compute = toUseGPU;  // gpu
    crnn_net->opt.use_fp16_arithmetic = true;  // fp16运算加速
    paramPath = [[NSBundle mainBundle] pathForResource:@"crnn_lite_op" ofType:@"param"];
    binPath = [[NSBundle mainBundle] pathForResource:@"crnn_lite_op" ofType:@"bin"];
    rp = crnn_net->load_param([paramPath UTF8String]);
    rm = crnn_net->load_model([binPath UTF8String]);
    if (rp == 0 && rm == 0) {
        NSLog(@"crnn_net load param and model success!");
    } else {
        NSLog(@"crnn_net load fail,param:%d model:%d", rp, rm);
    }

    angle_net = new ncnn::Net();
    angle_net->opt.use_vulkan_compute = toUseGPU;  // gpu
    angle_net->opt.use_fp16_arithmetic = true;  // fp16运算加速
    paramPath = [[NSBundle mainBundle] pathForResource:@"angle_op" ofType:@"param"];
    binPath = [[NSBundle mainBundle] pathForResource:@"angle_op" ofType:@"bin"];
    rp = angle_net->load_param([paramPath UTF8String]);
    rm = angle_net->load_model([binPath UTF8String]);
    if (rp == 0 && rm == 0) {
        NSLog(@"angle_net load param and model success!");
    } else {
        NSLog(@"angle_net load fail,param:%d model:%d", rp, rm);
    }
    
    //load keys
    NSString *keyPath = [[NSBundle mainBundle] pathForResource:@"keys" ofType:@"txt"];
    ifstream in([keyPath UTF8String]);
    std::string filename;
    std::string line;
    if (in) { // 有该文件
        ...
        NSLog(@"ocr keys.txt is not exists");
    }
...
std::vector<OCRResult> OCR::detect(UIImage *image, int short_size) {
    ...
    ncnn::Mat src_img = ncnn::Mat::from_pixels_resize(rgba, ncnn::Mat::PIXEL_RGBA2RGB, w, h, w, h);

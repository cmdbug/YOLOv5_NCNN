...
bool LightOpenPose::hasGPU = false;
bool LightOpenPose::toUseGPU = false;
LightOpenPose *LightOpenPose::detector = nullptr;

LightOpenPose::LightOpenPose(bool useGPU) {
#if NCNN_VULKAN
    ...
    // opt 需要在加载前设置
    humanPoseNet = new ncnn::Net();
    humanPoseNet->opt.use_vulkan_compute = toUseGPU;
    humanPoseNet->opt.use_fp16_arithmetic = true;
    NSString *parmaPath = [[NSBundle mainBundle] pathForResource:@"human_pose_sim_opt" ofType:@"param"];
    NSString *binPath = [[NSBundle mainBundle] pathForResource:@"human_pose_sim_opt" ofType:@"bin"];
    int rp = humanPoseNet->load_param([parmaPath UTF8String]);
    int rm = humanPoseNet->load_model([binPath UTF8String]);
    if (rp == 0 && rm == 0) {
        NSLog(@"net load param and model success!");
    } else {
        NSLog(@"net load fail,param:%d model:%d", rp, rm);
    }
}
...
void LightOpenPose::preprocess(UIImage *image, ncnn::Mat &in) {
    ...
    in = ncnn::Mat::from_pixels_resize(rgba, ncnn::Mat::PIXEL_RGBA2BGR, w, h, input_size_w, input_size_h);
